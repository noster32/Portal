...
public class CBall : CTeleportObject
{
    ...
    //Action => delegate랑 동일 predefined delegate
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.layer == LayerMask.NameToLayer("PortalCollider"))
            return;
        if (collision.transform.CompareTag("Pedestal"))
            return;

        if(collision.transform.TryGetComponent(out CPlayerState state))
            state.DealDamageToPlayer(999);
        ...
        CParticleManager.Instance.PlayParticle(sparkParticle, collision.contacts[0].point, 
                                               Quaternion.LookRotation(collision.contacts[0].normal), 0.2f);
    ...
    public void DestroyBall(Action<CBall> action) => isSpawnedFalseAction = action;
